...
2. **Tier defensibility.** A node marked **core** should be one the goal structurally depends on, or a synthesis point where separate strands combine — not just "important-feeling." A node marked **peripheral** should genuinely be skippable without breaking the path to the goal. Flag any node where the tier reads like a guess rather than a conclusion from the goal's actual dependency structure.
3. **Coverage gaps.** Check the graph against the understanding map for strands that show up as shaky or absent but have no corresponding node at all — a gap in the graph is as much a problem as an invented node in it.
4. **Ordering.** Flag any edge that has a node depending on something that comes later in the plan, or a node placed before a prerequisite the understanding map shows the learner doesn't have.
